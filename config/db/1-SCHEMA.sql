drop schema if exists `computer-database-db`;
  ...
  alter table computer add constraint fk_computer_company_1 foreign key (computer_company_id) references companyName (company_id) on delete restrict on update restrict;
